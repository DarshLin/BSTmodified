...
int BST<T>::recursiveLevelAct(BST<T>::BinNodePtr ptr, const T &item, int &level)
{
  if(ptr->data != item) {
	...
	if(item < ptr->data) {
	  if(ptr->left != NULL) {
	  level++;
	  recursiveLevelAct(ptr->left, item, level);
	  }
	  else {
		level = -1; //This will break out of the recursion because it has hit lower than far left
	  }
	}
	if(item > ptr->data) {
	  if(ptr->right != NULL) {
	  level++;
	  recursiveLevelAct(ptr->right, item, level);
	  }
	  else {
		level = -1; //This will break out of recursion because it has hit higher than far right
	  }
	}
  }
  return level;
...
int BST<T>::nonrecursiveLevel(const T &item)
{
  ...
  while (ptr) { //While there is still a ptr it will move
	...
	//Left Side
	if(item < ptr->data) {//much like the recursive function, it will move and add level every time
	  if(ptr->left != NULL) {
	  ptr = ptr->left;
	  level++;
	  }
	  else { //much like the recursion, it will break when it goes too far one way leaving the level at -1
		level = -1;
		break;
	  }
	}
  ...
  return level; //All level variables should be settled
  }
}
